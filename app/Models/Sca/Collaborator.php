...
class Collaborator extends Model
{
    ...
    // Para roles
    public function getEventsBk($registers) {
    ...
    public function getEvents($events, $registers) {
        return array_map(function($event) use ($registers) {
            return $this->getEvent($event, $registers->where('event_id', $event['id']));
        }, $events);
    }

    public function getEvent($event, $registers) {

        foreach ($event['register_types'] as $key => $register_type) {

            $register_first = $registers->first(function($register) use($register_type) {
                return $register->type_id == $register_type['id'];
            });

            if (!empty($register_first)) {
                unset($register_first['event']);
            }
 
            $event[$register_type['type']] = $register_first;
        }
